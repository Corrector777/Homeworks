...
# ingredients = ['хлор', 'тротил', 'нитроглицерин']  # Добавьте «запрещённую комбинацию» ингредиентов, приводящую к «взрыву» лаборатории.

# match ingredients:
#     case [_, "паучий глаз", _]:
#         print("Возможно, получится зелье невидимости")
#     case ['яд', _, 'трава']:  # Добавьте новый рецепт зелья, требующий точно два определённых ингредиента. Опять же открыт вопрос с последовательностью элементов в списке(строка 138)
#         print(f'для этого зелья необходимы: <яд> и <трава>')
...
#         print('лягушая лапка отсутствует, поэтому зелье работает')
...
#     case ['хлор', 'нитроглицерин', 'тротил']:  # вот тут интересно, а как сравнить 2 набора данных в разном порядке?! Вопрсо открыт. так как тут только данная последовательность отлавливается
#         print('Бабах')
#     case _:
...
match sensor_data:
    case {**rest} if not rest['fuel']:  # Проверка датчика топлива
        print('Датчик топлива не отвечает')
    case {**rest} if not rest['distance_to_destination']:  # Проверка датчика расстояния
    ...
    case {**rest} if rest["nearest_object"]:  # обнаружена космическая станция поблизости.
        print(f'Рядом обнаружено: {rest["nearest_object"]}')
    case {**rest} if rest["engine_status"] == 'перегрев':  # Перегрев двигателя
