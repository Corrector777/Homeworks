...
match ingredients:
    ...
    case [*rest] if {'яд', 'ухо'}.issubset(ingredients) and 'трава' not in rest:  # Реализуйте рецепт, который срабатывает только в случае отсутствия определённого ингредиента в списке.
    ...
    # case [*rest] if {'хлор', 'нитроглицерин', 'тротил'}.issubset(rest):  # Списки траснформируем в множества,чтобы не привязываться к порядку элемента. проверяем является ли захваченное множество подмножеством изначального множества
        # print('Бабах')
    case a if all(x in {'хлор', 'нитроглицерин', 'тротил'} for x in ingredients ):
        print('Генератор работает -  Бабах!!')
    case _:
