...
ingredients = ['хлор', 'тротил', 'нитроглицерин', 'мята']  # Добавьте «запрещённую комбинацию» ингредиентов, приводящую к «взрыву» лаборатории.
...
match ingredients:
    ...
    # case [*rest] if {'хлор', 'нитроглицерин', 'тротил'}.issubset(rest):  # Списки траснформируем в множества,чтобы не привязываться к порядку элемента. проверяем является ли захваченное множество подмножеством изначального множества
        # print('Бабах')
    case a if all(x in {'хлор', 'нитроглицерин', 'тротил'} for x in a):
